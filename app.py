...
def auto_detect_plate(cv_img, model_params, num_layers):
    ...
    bfilter = cv2.bilateralFilter(gray, 11, 17, 17) # Giảm nhiễu
    edged = cv2.Canny(bfilter, 30, 200) # Phát hiện cạnh
    
    contours, _ = cv2.findContours(edged.copy(), cv2.RETR_TREE, cv2.CHAIN_APPROX_SIMPLE)
    contours = sorted(contours, key=cv2.contourArea, reverse=True)[:50] # Lấy top 50 vùng lớn nhất
    
    candidates = []
    
    img_h, img_w = cv_img.shape[:2]
    # Biển số phải rộng ít nhất 4% ảnh và cao ít nhất 2% ảnh
    min_plate_w = int(img_w * 0.04)
    min_plate_h = int(img_h * 0.02)
    # Biển số không nên chiếm quá 90% ảnh
    max_plate_w = int(img_w * 0.90)
    max_plate_h = int(img_h * 0.60)

    for contour in contours:
        x, y, w, h = cv2.boundingRect(contour)
        aspect_ratio = w / float(max(1, h))
        
        # Biển xe VN: Vuông (~1.2-1.5), Dài (~2.0-5.5), kích thước đủ lớn
        if 1.0 < aspect_ratio < 5.5 \
                and min_plate_w < w < max_plate_w \
                and min_plate_h < h < max_plate_h:
            pad = 4
            rx1 = max(0, x - pad)
            ry1 = max(0, y - pad)
            ...
            if flat is not None:
                ...
                # prob của class 1 (Là Biển Số Xe gốc)
                prob_is_plate = probs[0][1] * 100
                pred_idx = np.argmax(probs[0])
                
                if pred_idx == 1 and prob_is_plate > 75.0:
                    candidates.append({
                        "box": (rx1, ry1, rx2, ry2),
                        "conf": prob_is_plate
                    })
                    
    # NMS (Non-Maximum Suppression) loại bỏ vùng trùng lặp
    ...
    for c in candidates:
        ...
        for f in final_boxes:
            if iou(c['box'], f['box']) > 0.1: # Trùng nhau > 10%
                keep = False
                break
        if keep:
...
class LicensePlateApp:
    def __init__(self, root):
        ...
        self.rect = None
        self.rects = [] # Lưu các vùng tự động vẽ
        self.start_x = None
        self.start_y = None
