"""
gui_plate_detector.py — v3 (Tất cả bug đã sửa)
=============================================================================
FIX so với bản cũ:
  🐛 FIX #9  (CRITICAL): segment_characters — trim >9 ký tự theo vị trí
                          outlier thay vì diện tích lớn nhất
  🐛 FIX #10 (CRITICAL): read_plate_text — thêm TTA inference (n_tta=3)
                          để khớp với accuracy đo trên test set
  🐛 FIX #11 (MEDIUM):   _find_char_contours — solidity threshold 0.08→0.15
  🐛 FIX #12 (MEDIUM):   _detect_two_rows — dùng threshold tương đối (peak*0.55)
                          thay vì absolute (w*0.35)
  🐛 FIX chung:           extract_hog_char — bilateralFilter sigma 50→30
  🐛 FIX model:           OCRNet đồng bộ với train_ocr.py v3
                          (residual connection, dropout thấp hơn, bottleneck 128)
=============================================================================
"""
...
def extract_hog_features(img_bgr):
    """Trích HOG cho plate detector — sigma giữ nguyên 50 (ảnh lớn hơn)."""
    ...
    gray    = cv2.bilateralFilter(gray, 5, 35, 35)   # FIX chung: 50→35
...
def extract_hog_char(img_bgr):
    ...
    🐛 FIX chung: sigma giảm 50→30 — giữ cạnh ký tự nhỏ tốt hơn.
    """
    if img_bgr is None or img_bgr.size == 0: return None
    h, w = img_bgr.shape[:2]
...
class OCRNet(nn.Module):
    """
    OCRNet v3 — đồng bộ với train_ocr.py v3.
    🐛 FIX #6: Dropout 0.3/0.25/0.2/0.1 (giảm từ 0.4/0.35/0.3/0.2)
    🐛 FIX #7: Bottleneck 128 (tăng từ 64)
    🐛 FIX #8: Residual Connection skip từ 256 → 128
    QUAN TRỌNG: Phải khớp CHÍNH XÁC với kiến trúc trong train_ocr.py
                để load_state_dict() không lỗi.
    """
    def __init__(self, input_dim, num_classes=NUM_CLASSES):
        ...
        self.fc2 = nn.Sequential(
            nn.Linear(512, 256),
            nn.BatchNorm1d(256), nn.SiLU(), nn.Dropout(0.25)
        )
        self.fc3 = nn.Sequential(
            nn.Linear(256, 128),
            nn.BatchNorm1d(128), nn.SiLU(), nn.Dropout(0.2)
        )
        self.fc4 = nn.Sequential(
            nn.Linear(128, 128),
            nn.BatchNorm1d(128), nn.SiLU(), nn.Dropout(0.1)
        )
        self.out  = nn.Linear(128, num_classes)
        self.skip = nn.Linear(256, 128, bias=False)   # Residual projection
...
def load_model_and_scaler(model_path="final_plate_model.pth",
                           scaler_path="scaler.pkl"):
    if not os.path.exists(model_path) or not os.path.exists(scaler_path):
        raise FileNotFoundError(
            f"Không tìm thấy file model '{model_path}' hoặc scaler '{scaler_path}'."
        )
...
def load_ocr_model(model_path="ocr_model_best.pth",
                   scaler_path="ocr_scaler.pkl"):
    if not os.path.exists(model_path) or not os.path.exists(scaler_path):
        print(f"⚠️ Không tìm thấy OCR model ({model_path}) hoặc "
              f"scaler ({scaler_path}). OCR sẽ bị tắt.")
        return None, None
    with open(scaler_path, 'rb') as f:
        scaler = pickle.load(f)
...
def is_plate_candidate(img_crop):
    ...
    valid_bg = ((white_mask > 0) | (yellow_mask > 0) | (blue_mask > 0) |
                (red_mask1 > 0) | (red_mask2 > 0))
    if float(np.sum(valid_bg)) / (h * w) < 0.20:
        return False
...
def post_nms_filter(img, boxes, scores):
    """Lọc sau NMS — hỗ trợ biển xanh / đỏ."""
    valid_boxes  = []
    valid_scores = []
    ...
    for i, (x1, y1, x2, y2) in enumerate(boxes):
        ...
        bg_ratio = float(np.sum((white_mask > 0) | (yellow_mask > 0) |
                                (blue_mask > 0) | (red_mask1 > 0) |
                                (red_mask2 > 0))) / (h_roi * w_roi)
        std_val  = float(np.std(gray.astype(np.float32)))
...
def segment_characters(plate_bgr):
    """
    Tách ký tự từ ảnh biển số VN.
    🐛 FIX #9: Trim >9 ký tự bằng cách loại bỏ position outlier
              (không phải loại ký tự nhỏ diện tích nhất — sai).
    """
    ...
    for th_raw in thresholds:
        ...
        if split_y is not None:
            ...
            combined += [(cx + 10000, roi)
                         for cx, _cy, _cw, _ch, roi in bot_chars]
        else:
            chars    = _find_char_contours(th_clean, plate_bgr, h, w,
                                           row_mode=False)
    ...
    # ──────────────────────────────────────────────────────────────────
    # FIX #9: Trim >9 ký tự theo vị trí outlier (không phải diện tích)
    # Ký tự thật sự nằm liên tục theo trục X; outlier = cách xa nhóm
    # ──────────────────────────────────────────────────────────────────
    if len(best_result) > 9:
        best_result = _trim_to_9_by_position(best_result)

    return best_result


def _trim_to_9_by_position(candidates):
    """
    Giữ lại tối đa 9 ký tự gần nhau nhất về vị trí X,
    loại bỏ các outlier có khoảng cách đột biến.
    Offset 10000 (hàng 2 của biển 2 hàng) được xử lý riêng từng hàng.
    """
    # Tách hàng trên (key < 10000) và hàng dưới (key >= 10000)
    row1 = [(k, r) for k, r in candidates if k < 10000]
    row2 = [(k, r) for k, r in candidates if k >= 10000]

    def trim_row(row, max_chars):
        if len(row) <= max_chars:
            return row
        xs = np.array([k for k, _ in row], dtype=float)
        # Tìm cụm liên tục lớn nhất (loại bỏ gap bất thường)
        sorted_row = sorted(row, key=lambda t: t[0])
        xs_sorted  = np.array([k for k, _ in sorted_row], dtype=float)
        gaps       = np.diff(xs_sorted)
        if len(gaps) == 0:
            return sorted_row[:max_chars]
        median_gap = np.median(gaps)
        threshold  = median_gap * 4.0  # Gap > 4x median → outlier

        # Tìm cụm liên tục dài nhất
        ...
        kept = sorted_row[best_start:best_start + best_len]
        return kept[:max_chars]

    # Phân bổ quota 9 ký tự theo tỉ lệ
    n1  = len(row1)
    n2  = len(row2)
    tot = n1 + n2
    if tot == 0:
    ...
    trimmed  = trim_row(row1, max(q1, 1))
    trimmed += trim_row(row2, max(q2, 1)) if row2 else []

    # Đảm bảo không vượt quá 9
    trimmed.sort(key=lambda t: t[0])
    return trimmed[:9]
...
def _detect_two_rows(gray, h, w):
    ...
    🐛 FIX #12: Dùng threshold tương đối (peak * 0.55)
                thay vì absolute (w * 0.35) — hoạt động đúng với biển tối.
    """
    ar = w / float(h)
    if ar > 3.5 or ar < 0.8:
        return None
    ...
    # FIX #12: threshold tương đối
    if peak == 0 or proj_smooth[min_idx] > peak * 0.55:
        return None
...
def _make_thresholds(blur, gray):
    """Tạo nhiều ảnh binary từ nhiều phương pháp threshold."""
    results = []
    ...
    th_inv_gauss = cv2.adaptiveThreshold(blur, 255,
                                          cv2.ADAPTIVE_THRESH_GAUSSIAN_C,
                                          cv2.THRESH_BINARY_INV, 19, 8)
    results.append(th_inv_gauss)

    th_inv_mean = cv2.adaptiveThreshold(blur, 255,
                                         cv2.ADAPTIVE_THRESH_MEAN_C,
                                         cv2.THRESH_BINARY_INV, 15, 10)
    results.append(th_inv_mean)

    th_inv_gauss2 = cv2.adaptiveThreshold(blur, 255,
                                           cv2.ADAPTIVE_THRESH_GAUSSIAN_C,
    ...
    th_norm_gauss = cv2.adaptiveThreshold(blur, 255,
                                           cv2.ADAPTIVE_THRESH_GAUSSIAN_C,
                                           cv2.THRESH_BINARY, 19, 8)
    results.append(th_norm_gauss)

    # POLARITY 3: CLAHE-based
...
def _find_char_contours(th, plate_bgr, h, w, row_mode=False):
    """
    Tìm contours hợp lệ là ký tự.
    🐛 FIX #11: Solidity threshold tăng 0.08 → 0.15
                (loại bỏ noise tốt hơn, không bỏ sót ký tự thật).
    """
    ...
    for c in cnts:
        ...
        # FIX #11: Solidity 0.08 → 0.15
        area = cv2.contourArea(c)
        if cw * ch2 > 0 and area / (cw * ch2) < 0.15: continue   # FIX
...
# ==============================================================================
# ĐỌC KÝ TỰ (OCR) — TTA INFERENCE
# ==============================================================================

def read_plate_text(ocr_model, ocr_scaler, plate_bgr,
                    min_conf=0.30, n_tta=3):
    """
    Tách ký tự → classify với TTA → sửa nhầm lẫn → format.
    🐛 FIX #10: Thêm TTA inference (n_tta lần noise trong feature space).
                Đồng bộ với cách evaluate trong train_ocr.py → accuracy thực
                sát với accuracy báo cáo.
    """
    ...
    for _sort_key, roi in chars:
        ...
        # TTA: average softmax qua (1 + n_tta) lần
        all_probs = []
        t_orig    = torch.tensor(fs, dtype=torch.float32).to(device)
        with torch.no_grad():
            all_probs.append(
                torch.softmax(ocr_model(t_orig), dim=1).cpu().numpy()
            )
            for _ in range(n_tta):
                noise   = np.random.normal(0, 0.008, fs.shape).astype(np.float32)
                t_noisy = torch.tensor(fs + noise, dtype=torch.float32).to(device)
                all_probs.append(
                    torch.softmax(ocr_model(t_noisy), dim=1).cpu().numpy()
                )

        avg_probs = np.mean(all_probs, axis=0)   # shape (1, 30)
        p = int(np.argmax(avg_probs))
        c = float(avg_probs[0, p])

        texts.append(CLASS_NAMES[p] if c >= min_conf else "?")
        confs.append(c)
...
def _correct_ocr_confusions(texts):
    """Sửa lỗi nhầm lẫn phổ biến theo vị trí ký tự."""
    if len(texts) < 4:
        return texts
...
class PlateDetectorGUI:
    def __init__(self, root, model, scaler, ocr_model=None, ocr_scaler=None):
        ...
        self.root.title("Trình Nhận Diện & Đọc Biển Số Xe VN (HOG + ANN v3)")
        ...
        self.btn_load = tk.Button(top_frame, text="📁 Tải Ảnh",
                                   command=self.load_image,
                                   bg="#e0e0e0", font=("Arial", 10, "bold"),
                                   padx=10)
        self.btn_load.pack(side=tk.LEFT, padx=5)

        self.btn_auto = tk.Button(top_frame, text="🔍 Nhận diện Tự động",
                                   command=self.auto_detect,
                                   bg="#4CAF50", fg="white",
                                   font=("Arial", 10, "bold"), padx=10)
        self.btn_auto.pack(side=tk.LEFT, padx=5)

        self.lbl_status = tk.Label(top_frame,
                                    text="Vui lòng tải ảnh để bắt đầu.",
        ...
        tk.Label(slider_frame, text="Ngưỡng Confidence:",
                 font=("Arial", 9), bg="#f0f0f0").pack(side=tk.LEFT, padx=5)
        ...
        self.lbl_threshold = tk.Label(
            slider_frame,
            text=f"({DEFAULT_CONFIDENCE_THRESHOLD*100:.0f}%)",
        ...
        lbl_instructions = tk.Label(
            self.root,
            text="Bấm '🔍 Nhận diện Tự động' để quét, "
                 "hoặc kéo chuột để khoanh vùng thủ công.",
    ...
    def load_image(self):
        ...
        try:
            ...
            canvas_w, canvas_h = (self.canvas.winfo_width(),
            ...
            self.canvas.create_image(canvas_w / 2, canvas_h / 2,
                                      anchor=tk.CENTER, image=self.img_tk)
    ...
    def auto_detect(self):
        ...
        self.lbl_status.config(
            text=f"Đang phân tích (ngưỡng {threshold*100:.0f}%)...",
            fg="orange"
        )
        self.lbl_result.config(text="Biển số: …")
        self.root.update()
        ...
        for (x, y, w, h) in rects:
            ...
            if y + h > img_h * 0.95 and h < img_h * 0.08: continue
            if y < img_h * 0.02 and h < img_h * 0.06: continue

            roi = self.img_original[y:y+h, x:x+w]
            if not is_plate_candidate(roi): continue

            pred, confidence = predict_plate_with_confidence(
                self.model, self.scaler, roi
            )
        ...
        if len(detected_boxes) > 0:
            ...
            final_boxes  = np.array(final_boxes_list) if final_boxes_list \
                           else np.array([])
        ...
        if len(final_boxes) == 0:
            self.lbl_status.config(
                text=f"Không tìm thấy biển số nào "
        ...
        for i, (x1, y1, x2, y2) in enumerate(final_boxes):
            ...
            has_text = (len(plate_text.replace("?", "")
                                       .replace("-", "")
                                       .replace(" ", "")) >= 4)

            color = ("#00CC00" if score >= 0.98
                     else "#66CC00" if score >= 0.95
                     else "#FFAA00")
            ...
            self.canvas.create_text(
                c_x1, c_y1 - 5, text=label_text,
                fill=color, font=("Helvetica", 11, "bold"),
                anchor=tk.SW, tags="auto_box"
            )

        if all_plate_texts:
            self.lbl_result.config(
                text="Biển số: " + "  |  ".join(all_plate_texts)
            )
            self.lbl_status.config(
                text=f"Tìm thấy {len(final_boxes)} biển số, "
                     f"đọc được {len(all_plate_texts)} biển!",
                fg="green"
            )
        else:
    ...
    def on_button_release(self, event):
        ...
        prediction, confidence = predict_plate_with_confidence(
            self.model, self.scaler, img_crop
        )
        plate_text, char_confs = read_plate_text(
            self.ocr_model, self.ocr_scaler, img_crop
        )
        ...
        if prediction == 1:
            if plate_text:
                ...
                self.lbl_status.config(
                    text=f"BIỂN SỐ  det={confidence*100:.1f}%  "
                         f"ocr={avg_ocr*100:.1f}%",
                    fg="green"
                )
                self.lbl_result.config(text=f"Biển số: {plate_text}")
            else:
                self.lbl_status.config(
                    text=f"DỰ ĐOÁN: BIỂN SỐ "
                         f"(Confidence: {confidence*100:.1f}%) "
                         f"— chưa đọc được ký tự",
                    fg="green"
                )
                self.lbl_result.config(text="Biển số: (?)")
            ...
            self.lbl_status.config(
                text=f"DỰ ĐOÁN: NỀN/RÁC (Confidence: {confidence*100:.1f}%)",
                fg="red"
            )
            self.lbl_result.config(text="Biển số: —")
...
if __name__ == "__main__":
    try:
        ...
        if ocr_model is not None:
            ...
            print("⚠️ OCR model không khả dụng — "
                  "chỉ phát hiện biển số, không đọc ký tự.")
